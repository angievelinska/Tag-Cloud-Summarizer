...
//      IteratorFactory.setProperties(props);
    ...
    DoubleVector query = docBuilder.buildVector(new BufferedReader(new StringReader(q)),
            new DenseVector(lsa.getVectorLength()));
    Map<DoubleVector, Double> searchResults = new HashMap<DoubleVector, Double>();

    for (int i = 0; i < 9; i++) {
      DoubleVector doc = lsa.getDocumentVector(i);
      double cosim = Similarity.getSimilarity(Similarity.SimType.COSINE,
              doc.toArray(), query.toArray());
