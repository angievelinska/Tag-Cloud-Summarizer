...
Various methods of evaluation are used in \gls{IR} systems. Two popular measures are \textit{precision} and \textit{recall}. \\

\section{Measures for evaluation}
The two most frequent and basic measures for evaluation of \gls{IR} systems are precision and recall. For the simple case of a system which returns a result set of document to a query, they are defined like:  \\
...
Evaluation of \gls{IR} systems: \\
\begin{itemize}
\item A document collection
\item A test suite of information needs, or queries
\item A set of relevance judgements, binary assessment: relevant or non-relevant for each query-document pair
\end{itemize}

In \gls{IR} systems relevance is assessed relative to an information need, not a query. A document is considered relevant if it addresses the stated information need, not only if it contains all the words given in the query~(Manning et al.~\cite{Mann08}). \\

When short queries (composed of one word for example) are posted, it is difficult to define the information need of users. But nevertheless, the user has one, and the retrieved search results are evaluated based on his or her information need. 
...
\textbf{TODO:} give a link to Unified API manual online in DocMachine. \\

Stop words and punctuation are removed. The data model used in DocMachine stores the documents as XML files, and CoreMedia \gls{CMS} Unified API\footnote{} is used to strip the markup, in order to access the plain text~(listing~\ref{strip_markup}). \\
